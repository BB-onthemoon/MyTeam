...
### Session 021 — 2026-06-09 — User Preferences ปรับขนาดตัวอักษร (StoreSalesReturnDoc, 4 spawn: A store / B ปุ่ม / C rem / E polish)
**ทำได้ดี:** อ่าน `task-context.md` ต่อ context ได้ทุก spawn ไม่ false-alarm (ระบบใช้รอบ 2 ได้ผลต่อจาก S019); สถาปัตยกรรมสะอาด — store `UserPreferences` validate localStorage ด้วย whitelist `includes()` ก่อน cast (กัน tamper/CSS-injection 100% เพราะ px มาจาก `FONT_SCALE_PX` lookup ไม่ใช่ string จาก storage), `effect()` ใน constructor (injection context) apply เข้า `<html>`; **rem conversion แม่นทุกจุด 19/19** (`Npx÷18`) + รู้จัก**คงกลุ่ม3 px** (border/stroke/shadow/breakpoint = hairline) ไม่ over-convert + จับ **connector ต้องอิงค่าเดียวกับวงกลม** (20px↔1.1111rem) ไม่งั้นเส้นหลุดตอน scale; ตัดสินใจ design ดีมีเหตุผล (flex+gap แทน btn-group เพราะพื้น navy, px ตัว ก ในปุ่มแยกจาก FONT_SCALE_PX จริง, `document` global เพราะไม่มี SSR); **Polish จับ `.user-edit-label` ที่มี issue เดียวกับ `.user-label` เอง** (ไม่รอให้สั่ง — consistent fix); แยก `viewChild`(เล็ก) ออกโดยไม่แตะ `ViewChild`(decorator) ระวังดี; build เขียวทุก spawn verify เอง
**ทำพลาด:** ไม่มีจุดพังจริง — QA ทั้งคู่ fix-then-ship ไม่มี must-fix ฝั่ง code; ที่ต้องแก้เป็น **bug เชิงดีไซน์ที่คาดเดายาก** (ironic a11y: label `0.6875rem` หดต่ำกว่า 12px ตอน sm scale) ไม่ใช่ความผิด coding — แต่ถ้า Mobius คิดเผื่อ "rem นี้ที่ scale เล็กสุดเหลือกี่ px" ตั้งแต่ Step C จะดักได้เองก่อน QA
**แนวทางปรับปรุง:** เวลาแปลง/ใช้ `rem` ในระบบที่ base scale ได้ ให้คิดเสมอว่า "ค่านี้ที่ scale **เล็กสุด** เหลือกี่ px" — label ต้อง ≥ 12px (floor `0.75rem` ที่ base 16px); และ **control ที่ปรับ font เอง (ปุ่ม segmented) ต้องคงเป็น px** ไม่ scale ตัวเอง (ทำถูกแล้วรอบนี้ — จำ pattern ไว้)
