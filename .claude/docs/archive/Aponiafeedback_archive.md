...
### Session 021 — 2026-06-09 — User Preferences font-size QA (StoreSalesReturnDoc)
**ทำได้ดี:** อ่าน `task-context.md` ก่อน → ไม่ flag invariant ที่ตั้งใจ (กลุ่ม3 px คงไว้, font-scale-btn 28px คงที่, toggle แทน radiogroup, fontScaleOptions.px≠FONT_SCALE_PX); **คำนวณ contrast จริงเป็นเลข** ทุกคู่สี (light/navy 15.36:1, ขาว/active-blue 6.64:1, active-border/navy 2.44:1<3:1) ไม่เดา; **รัน `ng build` จริงยืนยันเอง** ไม่เชื่อรายงาน Mobius อย่างเดียว (S011 lesson); วิเคราะห์ geometry connector เชิงเรขาคณิต (รัศมี = ครึ่งวงกลม scale พร้อมกัน) สรุปว่าไม่ใช่ bug; ยืนยัน security เชิง data-flow (px จาก lookup ไม่ใช่ storage string = กัน CSS injection); ซื่อสัตย์ว่า "ยังไม่เปิด browser จริง" flag baseline ไทย/flex-wrap เป็น runtime-check; verdict fix-then-ship เหลือแค่ dead import
**ทำพลาด:** ไม่มีข้อพลาดหลัก — แต่ **bug เด่นของงาน (ironic a11y: rem label หดต่ำกว่า 12px ตอน sm) Sakura จับ Aponia ไม่จับ** เพราะเป็นมุม design-constraint (12px min label) มากกว่า code/contrast
**แนวทางปรับปรุง:** ในงานที่มี global scaling (font/zoom) ให้ trace "ค่า rem แต่ละจุด ณ scale **เล็กสุด** เหลือกี่ px" เป็น a11y check ด้วย ไม่ใช่แค่ contrast/security — minimum readable size (12px) ก็เป็น a11y เหมือนกัน
